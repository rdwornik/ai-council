...
### 2026-07-27 (3) — RECORD TURN: the close-out audit lands, ADR-16 ratified, and the ADR needed three review passes to stop breaking its own rule (`0b14cef`)

**The audit that found the gap is now IN the repo, which was the gap.** The 2026-07-27 close-out audit is committed byte-verbatim at `docs/audits/2026-07-27-close-out-audit-execution-window.md` (sha256 identical on both sides). Its sharpest finding: the execution window asserted closure criterion 3 **MET** on review passes that existed **nowhere in the repository** — `git log 1dcee8b..17a239b -- docs/audits/` returns zero commits. A clone could not check the claim.

**ADR-16 — Evidence must be retrievable from the repository.** A repo record names a committed path, a sha, or a re-deriving command; **review outputs land in `docs/audits/` by the lane that ran them**; closure criteria are phrased so a clone can check them. **Clause 4 HOMES the #126 validator reporting contract**, which until now survived only inside `BACKLOG.md` — a log whose items *leave* under ADR-65 — carrying its hub-supersession trigger intact so it still expires by reference.

**The lesson of this turn is that ADR-16 broke its own rule three times before it held.** terra returned **three HIGH across three passes, all accepted**: (1) the ADR called the unrecorded reviews *"real"* and LESSONS said *"every word was true"* — **asserting, as established fact, a claim the repo cannot check, inside the ADR forbidding exactly that**; (2) its evidence command was **mutable** — a working-tree `grep` that this ADR's own arc falsifies, and which I confirmed was **already returning the audit file before merge** — plus one verb still asserting the review event; (3) clean. The repair is the useful part: the record now splits **retrievable EFFECT** (repair commits `8215ada`, `c687e81`, `d06f9dd`, `a33db9e`, `6b8160e`, `5757448`, all on `main`) from **unretrievable CAUSE** (the review outputs), pins evidence to `17a239b` instead of `HEAD`, and **attributes** the "13 findings across 9 passes" figure to the closure block's claim rather than asserting it. That an ADR about unretrievable claims needed three passes to stop making them is the strongest argument for it, and it is why **the review is on disk** at `docs/audits/2026-07-27-codex-close-out-records.md` — clause 2 discharged in the same commit that created the obligation.

**Filed: [#130], [#132], [#133]** — each id grepped repo-wide in **both** id spaces before assignment. **Four ids skipped and recorded non-assignable**: **#128** (hub renumber provenance here and in two `council_inbox/archive/` session plans), **#129** (named in an *immutable* audit as a discarded next-local-id estimate), **#131** and **#134** (hub space). Ids stay monotonic, so those are **gaps, not free ids**; next free **#135**. #130 carries the architect ruling that rule 4's **code is right and its spec imprecise**, with an explicit argument for why that is *not* a #125 violation — #125 was surface **count** (the code was defective); this is relation **precision** (the code encodes a measured finding the spec never captured).

**Three factual repairs from the audit:** the queue block no longer says "ruling owed" three lines above "the ruling was made"; #127's "pytest 931" is dated as a measurement rather than a live claim; #50's anchor tail corrected from "ONE-COMMIT" to the measured **two** — a close commit **and** its `--no-ff` merge, which is what a branch-then-merge ship rule always produces.

**ARCHITECTURE and CLAUDE reconciled to ADR-16.** ARCHITECTURE gains the reporting contract and the venv-explicit gate; **all FIVE** CLAUDE "pre-merge gate" enumerations were reconciled in the one commit, because partial reconciliation *is* the drift — v2.14 set that precedent for exactly this reason, and the #123 repoint had landed without touching any of them. Rules 3 and 4 stayed **PASS** afterwards, and rule 4 — the one widened this week — confirmed ADR-16 reached all four roster surfaces.

**Changes:** `docs/audits/` (+2: the audit, the review record), `docs/decisions/ADR-16-retrievable-predicates.md` (new) + README index, `LESSONS.md`, `BACKLOG.md` (3 filed, 3 repairs, reservations + grooming log; 75 → 78), `ARCHITECTURE.md`, `CLAUDE.md` (both re-stamped), `JOURNAL.md` (this). Commit `0b14cef`. **No code: no `src/`, no `scripts/`.**

**Next:** the reserved axis slots, still untouched — **#27** (operator-gated, eighth window), **#117**'s remaining ADR-11 amendment (architect-owed), **#103**, then the **1.1 unit #34 + #76 + #100**. #123's machine-level uninstall clause and #127's billed witness remain the operator's. This entry's own commit and merge are the structural tail #50 now describes accurately.

### 2026-07-27 (2) — EXECUTION WINDOW: the ruling landed first, then three defect lanes (`8caee48`, `fbf6711`, `010f0b9`, `422e4a3`, `d12b4d1`)

**All four closure criteria met, and criterion 1 was met BEFORE any code lane opened.** #117 was ruled — **NO: Lane A remains fire-and-forget by contract; ambiguity resolves at the boost stage via the clarify loop; interactivity stays a separable rider on its own ADR, opened only on evidence the clarify loop fails.** #113 was driven to its named exit (b) and **closed as deliberately-not-doing**, its exit clause quoted verbatim in the closing commit. That is the first window in eight where a non-delegable item was discharged as a repo record rather than a chat outcome. #117 stays open on exactly one clause — the ADR-11 dated amendment marker, deliberately not written because ADR edits were out of the turn's scope; **flagged as a follow-up question, not silently skipped.**

**LANDED. Four lanes, each with a named Codex review, merged serially.**
- **#126** — the four silent gates now print name, verdict, predicate and **item counts**. A zero-item run is finally distinguishable from a clean one. 13 tests RED first; terra returned **No findings**.
- **#125** — rule 4 read ONE roster surface where its spec claims **four**. Now four, **bidirectionally**, plus a **harness-generic read-set disclosure**: every rule line ends `reads: <surfaces>`, observed by the harness, never declared by a rule. **The live four surfaces came back clean — no doc was edited to make that so.**
- **#124** — a clean-room venv from the declared ranges **failed mypy with 3 errors**, exactly as the ticket predicted. Fixed `src/` against the newer API, then found the real cause: `google-genai>=1.55.0` spanned a **breaking** API change. Floor moved to `>=2.14,<3.0` — forward to the API src is written against, **not** to what this machine held (1.73.1).
- **#123 (PARTIAL)** — `check.ps1` now runs every tool through `.venv\Scripts\python.exe`, from the repo root, and fails loud without a venv.

**THE WINDOW'S OWN LESSON, and it landed on me four times.** Terra returned **13 findings across 9 review passes**; **11 accepted, 2 refuted with reproductions rather than arguments**. The accepted ones were not stylistic — three were **defects I introduced while fixing something else**, and two were the *same class this window exists to close, committed by me in the commit message*: I asserted check.ps1 restored the caller's cwd "on every path" when `exit` bypasses `finally`, then fixed the exit path and again claimed "every path" while the *terminating-error* path was still broken. **A guarantee asserted without the mechanism that provides it — twice, in a window whose thesis is that exact defect.** Both are now true by construction and witnessed on all three paths. The two refutations matter equally: a claimed U+00FA/U+00B7 mismatch (codepoints measured: `0xb7` both sides, terra agreed on re-check) and a claimed every-id-per-entry divergence (built terra's exact scenario; rule and evidence both returned `['ADR-99']`). **Review findings are evidence, not verdicts** — and reproducing them is what separates the two.

**Also repaired, unplanned:** a **pre-existing test-isolation defect** this lane exposed — a rule-2 test assigned `vc._canonical_docs` directly on the module instead of via `monkeypatch`, so it never unwound and pinned rule 2 to `["DOC.md"]` for every later test in the session. Invisible until a later test called `run_all()` on a real repo.

**NOT DONE, named rather than left to inference.** **#123's `pip uninstall ai-council` clause is DEFERRED** — machine-level, outside this repo, and the ticket itself calls it the operator's call. Until it runs, a bare `python -m pytest` in the primary still resolves through the system editable install, and the `conftest.py` guard's "fails loudly under bare python" half stays unproven. **#127 was filed** (consuming the next free id per the reservations note): the genai floor is proven for **typing**, not for **live behaviour** — every unit test mocks the provider, and witnessing it needs a **billed** call that must not be spent unattended. **The three standing axes did not move: #27 not run (eighth window), #103 not started, the Contract-Version 1.1 arc untouched.** This window was defect work by design, but the axes are still the axes.

**Changes:** `BACKLOG.md` (#117 ruling + #113 closed + #96 retired + #114 aligned; #126/#125/#124 closed; #123 partial; #127 filed; queue block updated; count 78 → 75), `scripts/` (four validators, `validate_claims.py`, `check.ps1`), `tests/` (five files, ~30 tests added), `src/ai_council/` (`inbox.py`, `gemini_research.py`), `pyproject.toml`, `JOURNAL.md` (this). Merges `8caee48`, `fbf6711`, `010f0b9`, `422e4a3`, `d12b4d1`.

**Next:** the reserved axis slots, untouched by this window — **#27** (operator-gated, eighth window), **#103** (architect-owed), then the **Contract-Version 1.1 unit #34 + #76 + #100, together**. Behind them: **#105**, **#119/#120**, unit (b), **#118**, and the two obligations this window created — #123's uninstall clause and #127's billed witness.

### 2026-07-27 (1) — Anchor the 2026-07-26 tail (`4d4bd87`, `6ed4bbd`, `0e60320`); queue block + three annotations land (record-and-report turn)

**ANCHOR — the previous window's unanchored JOURNAL tail, paid as this window's first act (closure criterion 2).** Each sha verified to resolve and to be an ancestor of `main` before being written here: **`4d4bd87`** (docs: session close — repair [#105]'s dangling ref, file [#126], record what did NOT land), **`6ed4bbd`** (its `--no-ff` merge), **`0e60320`** (Merge docs/0726-placement-and-tail — [#118]/[#125] to [S18]; the ADR-85 one-commit-tail record on [#50]). **This is structural debt, not drift, and it is deliberately NOT filed as a defect:** a JOURNAL entry cannot name the SHA of the commit that contains it, so every session's final commit is unanchorable by construction ([#50]'s W4 addition records the class). This window opened by paying the inherited tail, exactly as the close-out demanded, and closes this entry by creating another.

**Did (record and report only — no code, no `src/`, no `scripts/`; Codex lane: NONE, stated reason: record-only, no code impact, nothing to review):** landed the 2026-07-27 **Working-queue block** ([#102]'s record half — reserved axis slots #27 / #117 / #103 / the Contract-Version 1.1 unit #34+#76+#100; the defect queue behind them with #124 -> #123 serial as a correctness constraint; the four window-closure criteria recorded verbatim; the block's own NOT-mechanically-validated status disclosed in place — mechanism legs stay capped and unbuilt) and the **three review annotations**: [#104] criterion-(iii) status note with figures verified live this date (`SUMMARY: pass 4 | FINDINGS 0 across 0 rules | anchor-missing 0 | skipped(Unit2) 9 | errors 0` — the zero-findings gate sits at distance zero over an empty denominator), [#125] scope correction (1 roster surface, not 2; one-directional; the harness-generic disclosure disposition recorded and flagged for a ruling, not ruled), [#117] submitted-for-decision note (closure criterion 4 governs its outcome; Lane A has no ask-back channel; #113's both exits already named). Commit `128c679`, merge `bd6c88f`.

**Changes:** `BACKLOG.md` (queue section + [#104]/[#125]/[#117] annotations + grooming-log entry; count 78 unchanged, next free #127, reserved #107; `validate_backlog` now reports **8 themes** — the queue section parses as a theme, expected and disclosed), `JOURNAL.md` (this).

**Next:** the non-delegable front in queue order — #27 (operator-gated), the #117 ruling (criterion 4: rule it or record the refusal; silence is the eighth window), #103 (architect-owed); then the 1.1 unit #34+#76+#100 together. This entry's own merge is the new structural tail — anchoring it is the next session's first act.
...
**#124 — a reproducibility defect, filed rather than folded.** A clean-room `pip install -e ".[dev]"` resolves **mypy 2.3.0** and **google-genai 2.14.0**, and `mypy src/` then fails with 3 errors (`inbox.py:137`, `gemini_research.py:88`/`:96` — the genai 2.x union shape). The primary's gate is green **only because its interpreter happens to hold** mypy 1.19.1 / genai 1.73.1, versions nothing in the repo pins. Kept **distinct from #123** deliberately: #123 is *which interpreter runs the gate*; #124 is that **the declared ranges do not reproduce a green gate at all**. Same class as LESSONS 2026-07-26 — "the gate is green" published with its real predicate (*on this machine, with these accidentally-held versions*) left unstated. Ruled the same day that **#124 BLOCKS #123**, recorded in **both** tickets: #123 repoints `check.ps1` at a `.venv`, and a venv built from those floors is a RED gate, so #123 executed first breaks the green gate on the day it lands and the breakage presents as its own regression.

**#97 — the seven remaining Unit-2 stubs are HELD BY ARCHITECT RULING**, not scheduled: diminishing value against the mission. The checker already earns its keep on the drift classes that actually bit; the rest are speculative against a now-clean canonical surface. Revisit when a drift a held rule would have caught actually bites, then build that one rule, earned by the incident. **Held, not forgotten and not silently dropped** — they stay REGISTERED, so every run still prints them as SKIP inside the computed coverage block (`stubbed (9)`, `TOTAL 14 of 14`). That is precisely why registration was insisted on over deletion: a dropped rule vanishes from the report, a held one keeps saying "not checked". #119/#120/#105 stay filed and unscheduled under the same ruling.

**A rejected tool call that had already executed — recorded because it looks exactly like #121 and is not.** The `#124` merge sequence was reported as user-rejected, yet `main` was found at `e57a9d9` with the branch deleted and the push landed. The reflog is **contiguous and fully attributable**: commit 16:24:17 → checkout → merge 16:24:43 → `branch -d` → push, no gap, normal identity. So the rejection notice arrived *after* execution rather than instead of it. This is a distinct failure shape from #121's unattributed checkout, and conflating them would have sent the next session hunting a phantom concurrent writer. **The branch deleted was `docs/124-reproducibility-and-held-stubs`**, via `-d` (safe delete, refuses if unmerged); its commit `6bfb2a1` is an ancestor of `main`, so nothing was lost — named here because deletion is a standing hard trigger even when routine.

**Teardown.** `main..worktree-stream-b` was **empty** (everything merged) before removal — the precondition, checked not assumed. `.claude/worktrees/stream-b` removed and pruned; **every branch kept**, no `-D` run on anything, so `worktree-stream-b` still holds its commits. Verified after: `git worktree list` shows the primary only, `.claude/worktrees/` is empty, no orphan siblings beside the repo, tree clean and in sync.

**Changes:** `BACKLOG.md` (#124 filed, #97 held-stubs ruling, #124↔#123 blocking recorded in both directions — count 75 → 76), `JOURNAL.md` (this). Merges `cd54d9a` and `e57a9d9`; `main` clean and in sync.

**Next:** Stream B unit (b) — rule 1 + rule 14 leg (b) on the approved predicate — is unstarted and unscheduled. **#124 must land before #123.** The ADR-85 anchor debt this entry clears was carried across a session boundary because attribution derives from the checkout rather than the session (#423/#121), so a fresh session would have inherited a block on commits it did not author.

### 2026-07-26 (4) — Stream A merged; four rulings; Stream B unit (a): the harness fix (`27c7a32`, `70c48d4`)

**Did:** Merged Stream A under a **bracketed** merge, recorded four rulings, then opened Stream B with the harness fix. Merges `b6db79f` (Stream A), `af05131` (conftest guard), `6f85d8a` (#121), `27c7a32` (rulings); Stream B unit (a) is `70c48d4` on `worktree-stream-b`, **committed and stopped, not merged**.

**The bracket produced a false positive on its first real use, and the fix is now recorded.** `git merge -F -` is **not supported** — `git commit` reads a message from stdin, `git merge` does not — so the merge errored and never ran; `git log -1 --format=%P` then read the *unchanged* HEAD's parent, which differs from the recorded HEAD, and the bracket cried mutation. One-condition brackets **cannot distinguish "the hazard occurred" from "the operation never ran"**. Corrected to two conditions in order: **assert HEAD CHANGED, then assert the new first-parent equals the recorded value** (#121); the gotcha itself filed as **#122**, named unfiled in the last handoff supplement and now having cost time twice. Every merge since used the corrected form and reported OK.

**Promotion HELD, on evidence I went looking for rather than assumed.** Rules 3 and 4 both clear criterion (i) — each has a denominator independent of the artifact it audits — and both measured 11 on (iii). **Neither cleared (ii)**, demonstrated: a **dirty uncommitted** `.pre-commit-config.yaml` flipped rule 3 PASS→FAIL, and one **untracked** `docs/decisions/ADR-99-scratch.md` flipped rule 4 PASS→FAIL, while a fresh clone at the identical commit passed both. **#104 gains criterion (iv)** — for a gating rule `anchor-missing` is a FAILURE, not a WARN, because a reworded section heading otherwise lets the audited document silently un-gate the rule that checks it; WARN semantics stay for non-gating rules, so (iv) is a promotion-time status flip. **The measured 11 was never credit toward promotion:** (iii) is conjunctive with (ii), so it was eleven clean runs of a rule that could not satisfy the criterion it was counted against.

**Stream B unit (a) — fix the harness, not the rules.** `ctx.read` returned the working copy and `ctx.glob` hit the disk: **rule 2's #116 defect living one layer up**, still live in rules 3 and 4 and waiting to be inherited by all nine stubs. `exists`/`read`/`glob` now answer questions about **HEAD's tree**; disk access survives as `disk_exists`/`disk_read`/`disk_glob`, explicit and used by **no** rule leg — asserted by a test that greps the rule-leg region, so determinism is a property rather than a convention. Doing it now was one change; after Unit 2 it would have been a retrofit across thirteen legs. `_committed_paths` folded into `ctx.committed_paths()` — rule 2 had been carrying a second implementation of the same predicate, the exact duplicate-predicate shape the LESSONS entry warns about.

**Test-first, real RED recorded** on all six new tests, the acceptance failing as `{4: 'anchor-missing'} != {4: 'pass'}` — a dirty primary genuinely disagreeing with a clean clone at the same commit. **Frozen acceptance met live**, with BOTH perturbations present at once: rule 3 PASS, rule 4 PASS, `SUMMARY pass 4 | FINDINGS 0` unchanged, where before the change each perturbation flipped its rule to FAIL. The dirty-vs-clean clause is the whole point — the earlier determinism run compared two CLEAN checkouts, which is the same blind spot as the `ls-files` acceptance it was meant to close. 73 passed, ruff clean, `src/` untouched. Six fixtures now commit their files; the r8 dangling-SHA fixture still dangles, since committing the doc that CITES a sha does not make the sha reachable.

**Finding that is neither mine nor the harness's, reported not filed:** the fresh per-worktree venv resolved **different dependency versions** than the system interpreter — mypy **2.3.0 vs 1.19.1**, google-genai **2.14.0 vs 1.73.1** — and mypy fails there on 3 pre-existing `src/` errors from the newer genai interactions API. `check.ps1` is green on the primary only because the system interpreter happens to hold older versions. **The declared dependency ranges do not reproduce a green gate**, so any clean-room install of this repo fails mypy today. Adjacent to **#123** (which interpreter runs the gate) but distinct from it, and left unfiled pending an operator ruling.

**Changes:** `BACKLOG.md` (criterion (iv), bracket correction on #121, #122/#123 filed — count 73 → 75), `conftest.py` (new, the wrong-tree guard, RED-proven in the worktree), `scripts/validate_claims.py` + `tests/test_validate_claims.py` (the harness fix, on `worktree-stream-b`). `main` is at `27c7a32`, pushed and clean.

**Next:** Stream B unit (b) — rule 1 + rule 14 leg (b) as one unit on the approved predicate (Modules table lines 31–52, name from column 1, path from column 3, edge = any import whose target is an enumerated module regardless of location, `config` the forcing case). Then #114, #119/#120, #105. `worktree-stream-b` is 3 ahead of `main` and unmerged by design.

### 2026-07-26 (3) — Stream A record remainder + Stream B worktree verdict (branch `docs/f2-remainder`, commit `bffed5a`, commit-and-STOP, NOT merged)

**Did — Stream A, documents only, one commit `bffed5a`.** Scope was corrected on arrival: the eight rulings, the four forks and #111 had already landed in the F2 batch, so re-filing them was explicitly out — verified against `BACKLOG.md` before writing, and prompt **item 5 was DROPPED as already-filed** (the ARCHITECTURE base-relative shorthand is **#118**, filed the same day). Re-filing it would have been exactly the duplicate the batch exists to prevent. **The renumber arc CLOSED after being carried since 2026-07-08:** **#110 → #84**, **#128 → #85**. Every live `refs #96` was resolved in the same edit as the audit required — both are now **hub-qualified inside #84/#85** (`hub #96`, closed upstream 2026-07-20, HUB id space), kept qualified rather than dropped so the provenance survives. **#96's reservation is therefore discharged and #96 is FREE**: its only cause was those bare carries. #84/#85's reservations are discharged too — those ids are live tasks now, not free ids. **Audits and JOURNAL were deliberately NOT edited**: they are immutable / append-only, and their #110/#128 mentions are a correct record of the pre-renumber state, not drift.

**ADR-15 authored — the window's one real record gap.** The path-resolution ruling existed only in a commit message and a ticket. The ADR carries the three candidate models and why the first two failed; the `git ls-files` (**INDEX**) vs `git ls-tree -r HEAD` (**COMMIT TREE**) distinction **and why it was invisible to the acceptance test** — index and tree are equal on a clean tree, and a clean tree is what repo policy *requires*, so the acceptance compared two things policy guarantees are identical and no run of it could have gone red; the four declared bases, why `docs/decisions/` was **required** (allowlist reachability for the hub-routed `handoffs/`/`transcripts/`, not a widening of disk resolution) and that the fourth base is a **cost**, with an explicit "do not resolve future cases by adding bases"; `_R2_RUNTIME_PATHS` and why **no measurement of the old guard could have predicted it**; and the self-validating rule that every declared base must exist in the tracked tree. **Id derived, not guessed:** the local space is ADR-01..ADR-14 (the 93–101 hits are ECOSYSTEM ADRs owned by the hub), max **14**, next free **15**, zero references anywhere. **No doc-claim tracks an ADR count** — but **rule 4 tracks the ADR SET**, so CLAUDE §11 and `docs/decisions/README.md` were updated in the same commit and rule 4 stays PASS with `FINDINGS 0` held.

**#104 criterion (iii) amended to PER-RULE**, with promotion per-rule rather than all-or-nothing. Read report-wide it was absurd: a newly implemented rule finding a REAL defect would reset the promotion clock for every already-clean rule — punishing the checker for working, and creating a standing incentive not to add rules. A rule's clock also restarts when its **own** implementation changes. Measured by running **each first-parent commit's own checker** in a throwaway clone: **rule 3 = 11, rule 4 = 11** (the checker's entire life; both already clear >=5), **rule 8 = 11 raw but its clock restarts at `e45c193`** (#108 changed it) → **4, one short**, **rule 2 = 2 and restarting again** under ADR-15.

**LESSONS gains one entry for the CLASS, not the bug:** *a value published without the predicate that produces it cannot be checked* — five instances (a registry test parametrized over its own registry; ARCHITECTURE's edge count with no edge rule; R8's finding count with no defect unit; the hub's `logs/OVERRIDES.md` counting override events with **no cause field**, so the rate cannot separate "operator bypassed a correct gate" from "gate misattributed authorship"; and the acceptance that could not fail).

**Report (g) — the premise needed correcting.** `a54f8df → 8710b2c` was **mine**: my `docs/journal-0726-close` merge, run and reported last session with its push output. The genuinely unattributed event is a different reflog entry — **`HEAD@{6}` 13:48:32, `checkout: moving from fix/111-token-log-address to main`**, which I did not issue. It is a branch-pointer move, not a reset, so nothing was lost. It falls inside the `check.ps1` window, but **no in-repo cause exists**: no hook, script or pre-commit stanza executes a checkout (the two hits in `check_floor_hash.py` are message strings), every git call in `tests/` uses `-C <tmp repo>`, and the two `cwd=_REPO` scripts are invoked by neither `check.ps1` nor pytest. Cause is external to the repo — harness rewind, another terminal, or IDE git integration; not narrowable from inside. **Documented fleet precedent found:** the hub's `logs/OVERRIDES.md` records the ADR-85 gate firing twice on 2026-07-25 against *"a concurrent session's commits … which it checked out in the shared primary tree"*, and names the gate defect — session attribution derived from commits-ahead-of-base in a **shared** checkout cannot distinguish authors. Same class, sibling repo, already written down. **This is why `bffed5a` is NOT merged:** serial integration through primary is only serial if primary has one writer.

**Did — Stream B, the verdict.** Provisioned through the sanctioned path (`EnterWorktree`, never a raw sibling `git worktree add`) after the pre-flight `git worktree list` showed only the primary. The probe inside the worktree returned **`ai_council.__file__` = the PRIMARY checkout's `src/`** and **`sys.prefix` = the system `Python312`, not a venv at all**. Stopped before any pytest, as instructed. **Root cause is worse than "worktrees share the primary's .venv":** the editable install lives in the **system interpreter's** site-packages (`Python312\Lib\site-packages\ai_council-1.0.0.dist-info`), so it leaks into every cwd and every worktree regardless of venv activation. Built a per-worktree venv with its own editable install; it now resolves to the worktree's own `src/`, while bare `python` still leaks — so worktree work must call `.venv/Scripts/python.exe` explicitly. Bonus determinism evidence: the checker in this **third** checkout reports the same `pass 4 | FINDINGS 0`.

**`.worktreeinclude` — tested, not adapted by analogy.** The hub's is one line, `ecosystem/*/state.yaml` (its `audit-health` gate blocks on `repos registered (none)` without it). **Every ai-council gate passes exit 0 in a fresh unseeded worktree** (backlog, docs-registry, sealed-keys, audit-casing, floor-hash, claims), so ai-council needs no include file today. The latent case is `docs/audits/2026-07-17-epi1-archaeology/` — gitignored, absent in every worktree — which **#27 already predicts** would false-block worktree commits if the stale-row check is ever armed. That is what the file should contain, when it is needed.

**Changes:** `BACKLOG.md` (renumber + reservations discharged + #104 amendment + grooming), `LESSONS.md` (one entry + header stamp), `docs/decisions/ADR-15-path-claim-resolution-model.md` (new), `docs/decisions/README.md` (index row), `CLAUDE.md` (§11 roster) — all in `bffed5a`. `validate_backlog` OK (7 themes, 15 stories, **72** tasks; a renumber is net 0). Checker `pass 4 | FINDINGS 0`. No pytest, no import, no code touched — Stream A was documents-only by contract.

**Next / open state:** `bffed5a` is **unpushed on `docs/f2-remainder`, deliberately unmerged** pending the (g) resolution above. Worktree `stream-b` (branch `worktree-stream-b`) is **kept on disk** with its venv — it is the Stream B work surface, not a leftover; teardown belongs to integration. Reserved ids are now **#107 only**. Rule 8 needs one more clean first-parent commit; rule 2's window restarts at ADR-15.

### 2026-07-26 (2) — F2 ruling batch; checker reaches FINDINGS 0; guard rebuilt (merges `45c4124`, `a54f8df`)

**Did:** Four steps. **F2 — the ruling batch:** nine rulings plus four "cheap forks" converted from chat into BACKLOG records, one commit. R1 → **#115** (self-referential verification gets two homes, and is explicitly NOT a checker rule for v1). R2+R5 → **#104 rewritten**: #105 struck as a blocker, **#116** added, and the prose trigger "one clean window per rule" replaced — it named no window length, no defect unit and no checkout, so no run could satisfy or fail it. R3 → recorded on #116. R6 → grooming note only. R7 → **#117**, giving the ADR-11 decision-1 ruling an owner so #113 cannot sit blocked on an unowned precondition. R8 → scope narrowed by the tags. R9 → #73/#8/#87 ruled, plus the review-lane routing posture given a repo home. Forks: **#43** no-fallback sentinel, **#6** keep-as-watch, **#56** banner-not-rename, **#109** write-it-down. **#8 and #87 are executor rulings** under the batch mandate, flagged in-ticket as reversible while still BACKLOG records.

**Result — the checker reports FINDINGS 0 for the first time.** **#108 closed:** rules 2 and 8 now share ONE pure sentence-scoped predicate (R2 suppresses, R8 classifies on external attribution alone), three deliberately narrow classes, **no allowlist entry added** — pinned by a test. 4 → 1 findings. **#111 closed**, and it was not the stale-reference correction it looked like: both citations sat in **owner=hub** regions carried byte-verbatim, so the fix is a fleet-methodology divergence. Root cause is **position-dependence, not wording** — the bare path is right at the hub and wrong at every consumer, so no byte-verbatim template can be correct in both. Filed upstream the same pass as hub **#427** (hub main `863cb804`, pushed) and the local divergence **names #427 as its retirement trigger**, so it expires by reference. **#106 closed** — promoted to defending rule 12's exemption, verified by executing the printed command in both shells. **#116 closed** — the ruled resolution model.

**The guard measurement, and what it could not see.** Instrumented the rule-2 guard for one run without changing it: **110 tokens reached it, 85 survived, 25 dropped, and every one of the 25 failed existence** — zero harmless drops. Three populations: 16 base-relative shorthand for real subtrees, 6 ecosystem paths, 3 org/repo slugs. The rebuilt model resolves against git state under declared bases, with exclusions as **declarations** rather than side effects. **Two deviations, both stated rather than folded in:** a **fourth base** `docs/decisions/` was required or 2 of the 16 would still fire; and **`_R2_RUNTIME_PATHS`** (`output/`, `council_inbox/archive/`) was **not predictable from the drop-set at all** — those tokens never reached the old guard, because they exist on disk and the old disk check passed them. Changing a guard surfaces a population no measurement of the old guard can show; the drop-set was a complete measurement of the wrong thing.

**The near-miss worth keeping.** The frozen acceptance was met — this checkout and a fresh clone at the identical commit both reported `pass 4 | FINDINGS 0`, with `output/` and `logs/` present on one and absent on the other. **terra's pre-merge review then found the model still wrong.** It read `git ls-files`, which reports the **INDEX**, so a staged-but-uncommitted path satisfied a claim. Index and commit tree agree on a clean tree — **which is exactly why the fresh-clone acceptance passed while the defect survived**: the acceptance could not have caught it, and a green acceptance was mistaken for a correct model. Now reads `git ls-tree -r HEAD`, with a staged-path test. Same review caught the runtime-path self-test using `git check-ignore`, which consults untracked local and global ignore config — the checker had deliberately avoided that call for determinism and the **test reintroduced it**: a rule obeyed in the implementation and broken in its own test buys nothing. Its third finding (any-base acceptance masks a missing path at its written location) is the ruled model's inherent cost, filed onto **#118** with a required collision test rather than patched inside #116.

**#83 discharged — through "or its findings are filed", NOT because the surface was clean.** terra pass 3 finally ran and returned 2 High, both reproduced first-hand before filing: **#119** (`"AI cannot"` matches the marker `"i cannot"`, so a valid crux is classified MALFORMED and reported as retrieval-unavailable — invisible at the call site) and **#120** (`check()` docstrings "Never raises" while catching only `ProviderError`).

**Changes:** `BACKLOG.md` (9 rulings + 4 forks; #106/#108/#111/#116 closed, #83 discharged, #114–#120 filed), `scripts/validate_claims.py` (context predicate, resolution model, cross-shell quoting), `tests/test_validate_claims.py` (67 tests), `CLAUDE.md` (v2.15, `last_reviewed` 2026-07-26), `.methodology.yaml` (`claude-md-token-log-address`), two audits. Merges `45c4124` (F2) and `a54f8df` (the rest). `check.ps1` green, 885 passed. Hub: `#427` filed and pushed (`863cb804`) from an isolated worktree, since the hub's primary checkout had concurrent uncommitted work — provisioned per the hub's own `.worktreeinclude`.

**Next:** #104 is **promotable but not promoted** — criterion (iii) needs zero distinct defects across >=5 consecutive first-parent commits of `main`, and the first clean commit is the one that landed this. #118/#119/#120 are filed, not built. The renumber arc (#110→#84, #128→#85) is still outstanding.

### 2026-07-26 — #112 closed via pushed tags; #97 F1 registry repair (merges `e311e55`, `dccb4bc`)

**Did:** Four items in order. **(1) #112 protection re-founded on refs.** Tagged the three remaining dangling SHAs and re-founded the whole set: `archive/journal-26192dd` (created 2026-07-25, annotated, never pushed) was **deleted and replaced** — the namespace is now `archive/cited-<sha>` on the operator's ruling, because `journal-` implied the commits were journal entries and three of the four are 2026-07-08 QA probes. Created and pushed `archive/cited-26192dd` / `-6a44191` / `-0b77a67` / `-bd3a17c`, each annotation carrying its **own** retention predicate (citation `file:line` list, "reachable only via this tag", what the commit is, `refs #112`) so a reader who finds the tag needs neither the ticket nor this entry. `gc.auto` restored to the git default. **(2) #27 re-checked, nothing edited.** **(3) sol blind-derivation lane, then F1.** **(4) terra pre-merge review.**

**Result:** **#112 CLOSED.** `git describe --all --contains` resolves all four (was `fatal: cannot describe` on three, tag-only on the fourth); all four confirmed on `origin`. **Rule 8 went 5 findings → PASS/0**; SUMMARY `pass 3 | FINDINGS 4 across 1 rules | anchor-missing 0 | skipped(Unit2) 7 | errors 0` — the 4 remaining are all pre-existing R2 path drift. Why tags beat the config value: `gc.auto=0` lived only in `.git/config`, untracked, inherited by no clone and no worktree; a pushed tag is a real ref that survives all three. **JOURNAL untouched by the repair** — append-only, and its citations resolve as written now that the commits are reachable, so no correction entry was owed; the ticket's own cited lines (`:26`/`:788`/`:1947`/`:1949`) had already drifted to `:60`/`:822`/`:1981`/`:1983`, which is why the tag annotations date-stamp their line list as a snapshot. **#27:** API is still the default — verified first-hand at `ADR-12:33`/`:63`/`:76-79` and `READING-INDEX.md:29` — so the instrument stands as written and #27 remains the authorizing gate; recorded as a grooming note, with #27, `READING-INDEX.md`, `SCORING-SHEET.md` and ADR-12 all left untouched.

**F1 — #97 registry repair (gates (i) and (ii) MET; ticket stays OPEN):** `RULES` held **eleven** legs against a **fourteen**-rule spec — rules **1** and **7** absent entirely. Repaired test-first: the external expected-set test landed **before** the registration so the proof sits in history rather than being asserted after the fact (`RULES drifted from the #97 spec: missing [1, 7], unexpected []`), then rules 1 and 7 registered as `_unit2_stub` rows, then KNOWN LIMITATIONS gained a **computed** coverage block (implemented 4 / stubbed 9 / structural 1 / absent 0, TOTAL 14 of 14). The counts come from the run: a hardcoded roster would go stale the first time a stub is implemented, which is the exact drift class the checker exists to catch, so `absent` is a **live** number. Seeded-omission re-proof on the real registry: dropping the rule-7 row yields `missing [7]` from the test and `absent      (1): 7` from the report.

**Two independent derivations, deliberately:** sol (`gpt-5.6-sol`) re-derived the expected id set from #97's inline spec **alone**, explicitly fenced off from `scripts/validate_claims.py` and the test file, and returned `{1..14}`, count 14, contiguous, rule 12 identified as the non-leg exemption. Agreement between two blind derivations is what makes the denominator evidence rather than an echo — a single hand-copied literal would have been the same class of self-reference the repair exists to remove. terra (`gpt-5.6-terra`) reviewed the full diff pre-merge: **0/0/0/0, body read** (the console severity heuristic is a known false reading, so the audit body was read directly), confirming the expected set is not derived from `vc.RULES`, the coverage block cannot overstate, and the exemption is non-tautologically constrained.

**Residual disclosed rather than buried — filed as #114:** `_SPEC_RULE_IDS` is hand-copied from the #97 line, so a fifteenth rule in the SPEC would leave both literals at fourteen and every test green. F1 closed the registry-vs-spec gap and left the spec-vs-ticket gap open **on purpose**, with the report naming it and pointing at the id. #114 carries a fail-loud constraint on any future parser (a parse yielding ≠14 ids must be a CHECKER ERROR, never a silent pass) because a brittle regex over a 30KB prose line trades a *disclosed* gap for an *undisclosed false pass*. **#97 NOT struck** — its seeded-violation-of-each-rule clause needs the nine stubs implemented.

**Changes:** `BACKLOG.md` (#112 closed, #97 reconciled, #114 filed, 2 grooming entries), `scripts/validate_claims.py` (+2 stub rows, `_SPEC_RULE_IDS`/`_STRUCTURAL_EXEMPTIONS`, `_ids`, coverage block), `tests/test_validate_claims.py` (+4 tests, header test repointed), `docs/audits/2026-07-26-codex-97-registry-repair.md` (new), `JOURNAL.md` (this). Merges `e311e55` (#112 + #27) and `dccb4bc` (F1). `check.ps1` green (pytest + mypy + ruff); 48 tests in the claims suite; `validate_backlog` OK (7 themes, 15 stories, 71 tasks). Tags `archive/cited-*` ×4 on `origin`. `gc.auto` unset. Count 71 → 70 (#112) → 71 (#114); next free local id **#115**.

**Next:** #105 remains #104's other blocker (findings-vs-distinct-defects). #114 is filed but not built. The nine stubs are still stubs — registration disclosed them, it did not check them.

### 2026-07-25 — #97 reconciled to reality: two absent rules found (branch `docs/backlog-97-reconcile`, commit-and-STOP, not merged)

**Did:** Merged the filing batch (`6002aba`, `--no-ff`, preflight clean, branch deleted with `-d`), then answered four read-only audit questions on the merged tree. Three produced facts worth keeping. **(1) The cloud-run contradiction resolved, and it is a checker defect, not a disagreement:** the discarded Ultraplan session claimed `logs/TOKEN-LOG.md` "is not a live checker finding — the repo-rooted guard suppresses it"; our run reports it. Both were honest. `logs/` is **gitignored** (`.gitignore:48`, zero tracked files), so it exists on this primary checkout as untracked debris → R2's repo-rooted guard passes → finding fires; on a fresh clone `logs/` is absent → guard suppresses → no finding. **R2 is non-deterministic across checkouts for the identical commit**, which undercuts #104's per-rule clean-window trigger independently of #105/#108. #111 stands as filed. **(2) Scan surfaces are disjoint complements:** rule 2 reads `CLAUDE/ARCHITECTURE/VISION/CONTRIBUTING` + `protocols/*.md` (8 files) and deliberately excludes append-only logs; rule 8 reads `JOURNAL.md`/`BACKLOG.md` only. No file is in both — which is why R2 stayed at 4 while BACKLOG gained backticked non-resolving paths. `LESSONS.md` is in **neither** (it holds one non-resolving path, `tasks/lessons.md` ×2, and zero SHA tokens) though the exclusion comment names it, so the comment describes a symmetry the code does not implement. **(3) The arithmetic failure:** the report prints 11 rules (4 implemented + 7 SKIP), rule 12 is structural in the `Finding` model — **and rules 1 and 7 appear nowhere at all.**

**Result:** One commit, `f7f22db`, editing `BACKLOG.md` line 170 (#97) and nothing else. It now (a) **records Unit 1's landing** (`b94e8d1`: rules 2/3/4/8 implemented, rule 12 structural via the evidence-argv + `shlex` round-trip invariant, wired as a non-blocking report-only `check.ps1` section) instead of reading as unbuilt work; (b) **records the gap** — rules **1** (module-table completeness) and **7** (stamp honesty extended to ADR deployment stamps outside `canonical_freshness`'s A2 scope) are **ABSENT from the `RULES` registry, not stubbed**: no leg, no `_unit2_stub` row, no `rule_id=`, and KNOWN LIMITATIONS names only the seven stubs, so **no run discloses them**; outstanding is therefore **NINE, not seven**, and both earned-by drifts (`boost`/`crux_check` missing from all three ARCHITECTURE tables; the ADR-11 stamp predating its own amendment) are **unmonitored today**; (c) **amends the done-when**, whose first three clauses were already true for the implemented subset — a reader checking it against a live run could reasonably have struck the line. Two gates added: all fourteen rules registered with rule 12's structural realization as the one sanctioned exemption, and a **registry-completeness test asserting the `RULES` id set against an EXPLICIT expected set**. `validate_backlog` OK (7 themes, 15 stories, 71 tasks); diff = one hunk at `:170`.

**Root cause of the missed rules, recorded because it generalizes:** the registry test is `@pytest.mark.parametrize("leg", vc.RULES)` — defined **over the registry**, so it covers whatever is present and **structurally cannot detect an omission**. 41 new tests passed green while two rules were missing and would have continued to indefinitely. A self-referential completeness test has no denominator; only an **external expected set** closes it. Same shape as #97 rule 14 leg (a) being vacuous without leg (b) — validating a hand-maintained map against itself. Both my filing pass and the operator's approval treated "4 built + 7 stubs" as the whole set without ever summing it against 14.

**Changes:** `BACKLOG.md` (#97 line only — `f7f22db`), `JOURNAL.md` (this entry). Earlier this session: `6002aba` merged the filing batch to `main`; `gc.auto` remains `0` locally.

**Untouched:** #104/#105/#106/#108, the [S18] story line, every other task; no renumber, no new id (the reconciliation is #97's own scope). No code, no test, no ADR — the registry repair itself is #97's remaining work, deliberately not done here. The four dangling SHAs are again **not re-cited** in this entry.

**Next:** operator is the merge gate for `f7f22db`. Then: the registry repair under #97 (register rules 1 and 7, add the external-denominator test); consider whether #104's blocker list should also name R2's checkout-nondeterminism; the gotchas window; the `#110 → #84` / `#128 → #85` renumber.

### 2026-07-25 — Filing batch: one operator window → 12 tracked tasks (branch `docs/backlog-filing-batch-2026-07-25`, commit-and-STOP, not merged)

**Did:** Converted an 18-item operator window (6 decisions, 5 #97 Unit-1 checker follow-ups, 3 drift-triage findings, 1 obligation, 3 gotchas, 2 hub-space carries) into `BACKLOG.md` task lines. PLAN-first: every claim grep/read-verified against the live tree before proposing, not accepted from the prompt — which changed three of the filings. **Id-space swept before assigning** (the standing grep-before-assign rule): `#100`–`#106`, `#108`, `#109`, `#111`–`#113` free; **`#107` skipped and newly reserved** — `council_inbox/archive/2026-07-08T1817_..._prompt-standards.md:275`/`:381` cite it in the **HUB** id space, so a local assignment would silently capture those refs; this is the **third** hub/local collision (`refs #96`; the #110/#128 carries; now #107) and the reservation note records #97 rule 13 as the real fix with this as its concrete test case. `#110` skipped — already live in-file as a carried hub-id task.

**Result:** One commit, `42d0c92`. **12 ids filed:** `#100` verdict-package JSON Schema (1.1 cut alongside #34/#76, [S10]); `#101` boost sharpening-annotation block + `#113` the interactive clarify-loop ([S13]); `#102` next-up ranked queue with a session-close regroom advisory ([S15]); `#103` the #19 framing-defense DESIGN commission, explicitly not a build ([S9]); `#104`/`#105`/`#106`/`#108` the #97 Unit-1 checker follow-ups ([S18]); `#109` the undocumented `verify_*`/`validate_*` prefix convention ([S7]); `#111` CLAUDE.md's `logs/TOKEN-LOG.md` citation and `#112` the four dangling JOURNAL SHAs ([S15]). **5 rider edits, no ids consumed:** #80 → the CommonMark continuation rule; #81 → KEEP-SCANNER + hand-ported fence-skip (tag `spike/md-parser-evidence`, verified present); #92 → the ARCHITECTURE allowed-edge re-derivation, the FLOOR1 R1 trigger (`b1f3319`) that sat at `ARCHITECTURE.md:180-181` as doc prose with **no task carrying it**; #19 → design-predecessor pointer to #103; #50 → the W4 watch candidate. `validate_backlog` **OK (7 themes, 15 stories, 71 tasks, 0 warnings)**; `git diff --stat` = `BACKLOG.md` alone, 1 file changed; all pre-commit hooks passed. Count **59 → 71**, stories **15**, next free **#114** (reserved #84/#85/#96/#107).

**Ruled this window, recorded so the reasoning survives:** (1) **#101/#113 filed as two tickets, not one with a rider** — "rider" named the relationship (`refs #101` carries it); a clause inside #101 would either block that line's closure or vanish with it, carrying no open-id status and unable to enter the queue, and "split it out when the ruling lands" is an instruction with no mechanism. #113 names **both** exits (built per Design B, **or** the ADR-11 decision-1 ruling forecloses interactivity and it closes as deliberately-not-doing) so a ruling either way resolves it rather than leaving a zombie. (2) **#104 blocked by #105 AND #108** — a "0 findings" gate is unpassable while four dangling SHAs yield six findings and self-replicate, and R2 cannot earn its per-rule clean window while three of its four first-run findings are its own context-blindness false positives. (3) **#102's queue uses a numbered-list shape** (`1. #100 — label`, never `- [#id]`), sidestepping both `validate_backlog` parse failures — `_THEME_RE` would read `## Next up` as a theme and any `- [#id]` inside it as a task, hard-failing twice — with **no grammar change and therefore no ADR-78 hub coordination**.

**Corrected on evidence, not accepted as given:** `protocols/AI_COUNCIL_PROCESS.md` arrived in the window as genuine drift and is a **checker false positive** — the file exists at `.dev-knowledge/protocols/AI_COUNCIL_PROCESS.md` and `protocols/README.md:20` attributes it to the hub *in prose*, while `_R2_ALLOWLIST` keys on the `.dev-knowledge/` prefix *inside* the backticks. Folded into #108 as leg (b), not filed as drift. Separately, the `logs/TOKEN-LOG.md` finding reconciled cleanly as **stale addressing, not a missing file**: `git log --all` on the path is empty (never existed here), the log is the hub's exactly as `INSTALL.md:33` already states, and that hub file is what `/session-summary`'s preflight read.

**Mechanism, not prose:** `gc.auto` set to **0** repo-locally (`.git/config`; previously unset = git default 6700) **before** the branch was cut. #112's "do not run gc" cannot stop `gc --auto` firing after commit/merge/receive-pack, and the four unreachable commits predate git's two-week default prune horizon, so age no longer protects them. Verified holding after the commit: config still `0`, all four still resolve as commit objects. Restoring `gc.auto` is part of #112's done-when so the repo does not carry silent disabled-maintenance debt. **The four SHAs are deliberately NOT re-cited in this entry** — they are enumerated in #112; re-citing them here is precisely the self-replication #105 names, and the Unit-1 entry above already demonstrated it by citing one as rule-12 evidence.

**Changes:** `BACKLOG.md` (12 task lines, 5 rider edits, extended id-reservations note, one grooming-log entry — `42d0c92`), `JOURNAL.md` (this entry), `.git/config` (`gc.auto=0`, local, uncommitted by nature).

**Untouched (surface honored):** every file but `BACKLOG.md` in the batch commit — no code, no ADR, no audit, no `~/.claude/` edit. The three gotchas (`git merge -F -` rejects the stdin dash unlike `git commit`; a called script's exit leaks into `check.ps1`'s exit so an explicit `exit 0` is required to keep a section non-blocking; ADR-85's gate fires per session-STOP, not per substantive change) route to `~/.claude/skills/gotchas/gotchas.md` in a separate window — deliberately not filed into BACKLOG. Two hub-space items carried out, not filed: the `grep -c '<<<<<<<' JOURNAL.md` counter inflating on its own logged quotation (verified: `1` unanchored vs `0` line-anchored), and the `Finding`/`RuleResult` divergence from hub #89's scalar `ClaimResult` (deliberate — a scalar cannot carry rule 12's per-finding evidence command).

**Process note (ADR-85, and item 18 firing live):** this entry exists because the session-end gate hard-blocked on an unanchored commit, and the window's own stated surface was `BACKLOG.md`-only. Operator ruling, now standing: **surface constraints scope the WORK, never session hygiene — JOURNAL and BACKLOG are governed by their gates, not by a prompt's file list.** The gate firing per session-STOP rather than per substantive change is the exact drift the W4 watch candidate filed on #50 this window describes.

**Next:** operator is the serial merge gate — `42d0c92` + this anchor await review, not merged, not pushed. Then: the gotchas window; the `#110 → #84` / `#128 → #85` renumber (must resolve all 5 `refs #96` in the same pass); `#8`/`#73`/`#87` done-when text pending ruling.

### 2026-07-24 — #97 Unit 1: claim-vs-reality checker (branch `feat/validate-claims-unit1`, commit-and-STOP, not merged)

**Did:** Built `scripts/validate_claims.py` (#97/[S18] Unit 1) — a read-only claim-vs-reality checker surfaced as a **non-blocking** `check.ps1` section, per #73 (standalone, not a pre-commit gate). Froze the skeleton for Unit 2: `Finding` (evidence is an argv tuple whose shlex-printed form round-trips back and is EXECUTED by the harness → rule-12 "re-runnable by a human" verified, not promised), `RuleResult` (pass/fail/anchor-missing/skipped — a reworded doc degrades to a WARN, never a synthesized finding, borrowed from the hub #89 precedent found live at `../.dev-knowledge/scripts/validate_doc_claims.py`), the frozen leg signature `check(ctx: RepoContext) -> RuleResult`, and exit codes 0/1/≥2 (a crashing leg forces ≥2, surfaced Red; the call site swallows via explicit `exit 0`). Implemented rules **2/3/4/8** RED-first (luna-sourced drift fixtures, CC-transcribed); **5/6/9/10/11/13/14** are Unit-2 skipped stubs. Each leg was FP-checked against the live repo mid-flight: R3's structural leading-bullet parse false-positived `ruff`/`toc-generate` (fixed → whole-doc word-boundary mention); R8's `for-each-ref --contains` per SHA was ~90s (fixed → one `rev-list --all` set + one `cat-file --batch-check` → 0.57s); R2 over-reported ecosystem paths (fixed → repo-rooted guard + minimal ADR-42/43 allowlist). C13 same-commit: the eight "gate = pytest+mypy+ruff/trio" enumerations a 4th section would stale (CLAUDE ×5, ARCHITECTURE ×2, CONTRIBUTING ×1) reconciled; CLAUDE/CONTRIBUTING `last_reviewed` → 2026-07-24, CLAUDE §12 v2.14.

**Result:** Two commits — `66bb5b1` (Unit 1) + `56f02dd` (terra review fixes). terra (codex) returned **5 High / 0 Critical** on `66bb5b1` (`docs/audits/2026-07-24-codex-validate-claims-unit1.md`; console 0/0/0/0 is the known regex defect, body read): H1 (git-failure masked as pass → now raises → exit ≥2), H2 (bare-runner evidence → now ≥2 tokens), H4 (leg/evidence scope mismatch → both whole-doc word-boundary; also fixes an ADR-1-vs-ADR-13 substring hazard), H5 (R8 evidence HEAD-only → `git describe --all --contains`, faithful any-ref) all fixed; H3 (R2 repo-rooted guard suppresses top-segment typos) dispositioned as a deliberate precision-over-recall tradeoff (documented, operator's call). Suite **859 passed, 6 deselected, 1 xfailed** (866 collected = 825 baseline + **41** new); `check.ps1` exit 0. The non-blocking checker surfaces **9 candidate findings on the live repo** (R2=4, R8=5) — genuine pre-existing drift incl. `logs/TOKEN-LOG.md` absent (CLAUDE §5 calls it append-only), `protocols/AI_COUNCIL_PROCESS.md` dangling, and 5 dangling JOURNAL SHA citations — surfaced not fixed (non-gating). Rule 12 proven by hand: `git describe --all --contains 26192dd` → exit 128. **Pre-merge (`0362d3d`):** C1 substance check (scratch, reverted) — each leg stubbed to a clean pass, every leg's drift test failed → tests non-tautological, C1 satisfied; added a KNOWN LIMITATIONS header to the checker's report (R2 precision-over-recall; unparsed-negation `.claude/skills/` **named not allowlisted** per operator ruling; Unit-2 SKIP caveat). Suite 860 passed / 867 collected; `check.ps1` exit 0. Confirmed fact: the checker's `logs/TOKEN-LOG.md` finding is real — the token log lives at the hub (`.dev-knowledge/logs/TOKEN-LOG.md`); CLAUDE §5:73/§10:173 cite it as a bare repo-relative path, unqualified.

**Changes:** `scripts/validate_claims.py` (new), `tests/test_validate_claims.py` (new, 41 tests), `scripts/check.ps1` (+non-blocking section, `exit 0` swallow), `CLAUDE.md`/`ARCHITECTURE.md`/`CONTRIBUTING.md` (C13 + freshness), `docs/audits/2026-07-24-codex-validate-claims-unit1.md` (review record).

**Untouched (forbidden surface honored):** `BACKLOG.md` (filing batch owns it — #97 advanced not closed, Unit 2 remains), ARCHITECTURE Layer-edge allowed-set (FLOOR1 `b1f3319`), the frozen #27 corpus, and every committed audit/JOURNAL entry (this is a new prepend, not an edit).

**Abandoned:** the per-token `for-each-ref --contains` reachability approach (perf); R3's structural leading-bullet roster parse and its two-way doc-only direction (FP-prone → whole-doc mention).

**Next:** operator merge-gate review of the two commits; ruling on H3 + the R2 `.claude/skills/` self-negating residual; Unit 2 (rules 5/6/9/10/11/13 + rule 14's two legs, incl. AST import-graph) against the frozen interface.

### 2026-07-24 — FLOOR1 R1: ARCHITECTURE allowed-edge-set marked TARGET (branch `docs/architecture-allowed-set-correction`, commit-and-STOP, not merged)

**Did:** R1 ruling (operator-confirmed 2026-07-24) — made ARCHITECTURE.md's Layer-edges section factually honest **without widening the allowed set** (precedent: rewording a boundary to match a defect launders the defect). Surface was **ARCHITECTURE.md ONLY**. Re-derived `cli.py`'s real edge surface **LIVE via AST** (stdlib scratch, not committed; `ast.Import`/`ast.ImportFrom`, TYPE_CHECKING-guarded imports excluded) rather than trusting the probe's remembered numbers — the derivation **reproduced 14 real edges / 3 covered / gap 11** exactly (3 inside the allowed set `interface->orchestration`: orchestrator/runner/doctor; 1 named open case `cli->boost`; 10 unaccounted across `interface->interface/core/foundation/output`). Edits in **`6b0ae40`**: allowed set relabelled **TARGET, not current state** (reusing the invariants-2/3 vocabulary from the 2026-07-23 repair, not a new form) with set VALUES byte-unchanged; a dated current-state note listing the derived edges by layer pair; the codemap-completeness gap recorded (11 real `cli` edges — the 10 unaccounted + `cli->runner`, allowed-but-unmapped — absent from the hand codemap) with a pointer to **#97 rule 14 leg (b)** (map-vs-source); "Utility-exemption modules: none" retained as TARGET; rule-14 enforcement note stated as two legs; `last_reviewed` 2026-07-23 → 2026-07-24 (A2 freshness, backed by a genuine re-read of the edited section). **Re-derivation trigger = after #92 lands.**

**Result:** Single-file diff (ARCHITECTURE.md, 41+/7−). All five read-only validators exit 0 (canonical_freshness, validate_backlog OK 7/15/59, validate_audit_casing, validate_docs_registry, validate_sealed_keys). **Frozen acceptance met:** set values unchanged (8 allowed-edge bullets are context-only in the diff), TARGET label + dated current-state note present, `cli->boost` still the one open case (unruled, bound to #69/P2). This JOURNAL anchor is a **separate commit** on the same branch — `6b0ae40` was NOT amended, so the operator's diff review + frozen-acceptance verification against it stay valid.

**Untouched (forbidden surface honored):** allowed-set values (no widening), the `cli->boost` open case, the Modules-table header (cosmetic — separate item), invariants 2/3 wording, and every other file including BACKLOG.md.

**Abandoned:** none.

**Next:** operator is the merge gate (commit-and-STOP). **BACKLOG advisory stands, correctly** — FLOOR1 finishes no tracked task, so no structural marker is due; the one obligation this correction creates — **re-derive the allowed set after #92 lands** — gets a home in the filing batch, not on this branch (BACKLOG edits stay serialized). Follow-up codemap edit (the 11 absent `cli` edges) rides the same #97/#92 arc, deliberately not applied here.

### 2026-07-24 — STEP-0 acceptance-freeze reads + #27 blind-scoring instrument prep (branch `docs/cli4-parity-scoring-prep`)

**Did:** Two parallel read/prep-only lanes, no build. **Lane 1 (STEP-0, read-only):** paste + trace-check ahead of the #97 checker acceptance-contract freeze. Established that `2026-07-22-pre-handoff-cleanup.md` §3 is **twelve** rules on disk (the "fourteen-rule spec" lives only in BACKLOG #97); confirmed rules 13/14 both carry **real, non-vacuous earned-by traces** in #97 (not backfilled) — with the honest caveat that they were earned in their own authoring session (architecture-repair pass) rather than a prior independent audit, and two are partly self-referential. Delivered #97/S18 + the three awaiting-ruling items (#8, #73, #87) in full; recommended the report-only checker home as a standalone `scripts/verify_claims.py` surfaced as a **non-blocking findings section in `check.ps1`** (trigger aligns to merge; Stop-hook rejected as constant-noise; noted #97's "home per #73" clause conflates the codex-review runner axis with a Python-script home). **Lane 2 (#27 prep, delegated subagent):** corpus at `docs/audits/2026-07-18-cli4-parity/` verified complete + internally consistent — **not repaired** (frozen); sealed key verified safe (`.gitignore:66`, untracked via `git check-ignore`/`git ls-files`; never opened/printed/restaged); created `READING-INDEX.md` (one-stop instrument, rubric inline, margin rules 1/12 overall + items 2/4 zero-margin, 12 pairs in scoring order) + `SCORING-RECORD.md` (blank 12-row which-wins + post-unseal tally) inside the existing workspace, no new folders.

**Result:** Lane 2 **READY**. Both prep files committed **`56a57a0`** and merged `--no-ff` **`766241a`**; all pre-commit hooks passed; tree clean; not pushed. Self-caught defect: first commit used PowerShell here-string syntax (`@'…'@`) in the Bash tool (POSIX sh), polluting the subject with a stray `@` — amended from a file before the merge; landed subject clean.

**Changes:** `docs/audits/2026-07-18-cli4-parity/READING-INDEX.md` + `SCORING-RECORD.md` (new), this entry. Frozen corpus, `SCORING-SHEET.md`, README, and `SEALED-KEY.json` untouched.

**Abandoned:** none.

**Next:** operator scores the 12 pairs (READING-INDEX top-to-bottom → SCORING-RECORD + per-item SCORING-SHEET), then unseals `SEALED-KEY.json` for the parity tally — #27 stays open until the scored parity ruling; #66 remains gated behind it. #97 acceptance contract is frozen; build not started.

### 2026-07-23 — Repair-pass merge + rule-14 de-vacuation + map-vs-source falsification (branch `chore/rule14-legs`)

**Did:** **(1)** Operator-approved merge of `docs/architecture-repair` → **`fc71328`** (`--no-ff`), pushed `fe42bc2..fc71328`, branch deleted; validators 0/0/0/0, 825 collected. **(2)** **#97 rule 14 given two legs** per operator ruling (as first written it was vacuous — leg (a) codemap-vs-allowed-set validates the hand-maintained map against itself, so an illegal import the map omits passes the gate, the exact case it exists to catch): leg (b) = every real `src/` inter-module import appears in the codemap (map completeness against source). **(3)** Operator's two-edge hypothesis grep-tested against `cli.py` — **both confirmed and the reality is larger**: `cli -> inbox` (`cli.py:23`, interface→interface) and `cli -> config` (`cli.py:37`, interface→foundation) are real, and the full import list shows **~11 real module edges absent from the codemap** — inbox:23, healthcheck:22, mode_detector:24, models:25, output:27, policy:28, providers:29-34, routing:35, runner:36, config:37, research (function-level, :103); research.models:19 is TYPE_CHECKING-only. So ARCHITECTURE's "`cli -> boost` is the only edge that skips a layer" is FALSE against source (it was verified map-vs-map — the exact vacuity leg (b) closes), the allowed edge set is a map-derived understatement, and "Utility-exemption modules: none" is falsified by cli's foundation imports. **Report-only per instruction — no ARCHITECTURE edit applied; the follow-up edit awaits the operator's ruling.**

**Result:** validate_backlog OK 7/15/59; leg (b) earned its keep the hour it was specified — live evidence recorded in #97's earned-by trace and the grooming log.

**Changes:** `BACKLOG.md` (#97 rule-14 two-leg rewrite + grooming entry), this entry.

**Next:** operator rules on the ARCHITECTURE allowed-set follow-up (cli's real edge profile: interface→interface/orchestration/core/foundation/output) — likely alongside the cli->boost open case in the #69/P2 arc, since #92's `cli.run()` refactor is what would actually shrink cli's import surface.

### 2026-07-23 — ARCHITECTURE.md architect repair pass (branch `docs/architecture-repair`, commit-and-STOP, not merged)

**Did:** Applied the architect's eight surgical edits (authored from a full read; exact old→new blocks) in commit **`f7cff47`**. E1: the layer-edge set **completed** (it omitted `orchestration->orchestration/foundation`, `core->core`, `output->output` — an incomplete set cannot gate) and **`cli -> boost` named as the one OPEN CASE** (`interface -> core`, unruled; resolutions (a) reclassify boost to orchestration / (b) admit scoped `interface->core`; ruling bound to the #69/P2 wiring arc, deliberately NOT silently legalised). E2/E8: invariant 2 + its Key-conventions echo marked **TARGET, not current state** (#92). E3: invariant 3's known exception recorded (research-provider constructor fallbacks; #99). E4: the boost input stage added to Data Flow as a **separate invocation** with the P2 not-yet-wired note (T8 strict xfail pins parity). E5 `[VERIFY]` **confirmed exactly**: briefs default to `./output` (`cli.py:971`), `council-brief-{ts}-{slug}`, legs `-1-research`/`-2-decision`, collisions `-v2`/`-v3` (`boost.py:474-480,561`). E6 `[VERIFY]`: ADR-13 = invocation-contract versioning, restored to the Governing-ADRs roster (CLAUDE.md's sibling was repaired a day earlier — live #97 rule-4 instance). E7 `[VERIFY]` case (i): ADR-01 amendment verified landed (`Revised 2026-07-18` in body, `e3bdcc8`/`a854bd3`; #2 struck verified-done) → stale `pending #2` clause deleted. While applying E1, every codemap edge was audited against the completed set — `cli -> boost` is genuinely the only edge outside it. Last-updated parenthetical extended in place; `last_reviewed` already 2026-07-23, not re-bumped.

**Result:** validators 0/0/0/0 (backlog OK 7/15/59); 825 collected; diff 73+/10− in `ARCHITECTURE.md` only. **Flagged residue:** E1's authored text cites "BACKLOG #97 rule 14" — #97 enumerates rules (1)–(12), so the forward reference dangles until #97 gains the layer-edge-conformance rule (one-line grooming fix). E4's exit-3-on-fallback claim applied on the architect's authority, not independently witnessed this pass. Cosmetic: Modules-table header still claims source root `src/ai_council/` while the config row correctly points at repo-root `config/`.
...
**Did:** **(1)** Merged `chore/session-close-absorb` → **`95b9e4f`** (`--no-ff`), pushed `eb82fd5..95b9e4f`; deleted both approved local branches (`chore/session-close-absorb`, `feat/vscode-boundary-colors` — the latter fully merged, ancestor of main); `main` is the only local branch. **(2) #96 collision verified first-hand, then repaired:** repo-wide grep confirmed #110/#128 carry `refs #96` in the **hub** id space (closed upstream; 2026-07-20 backlog audit A1 called it dangling) — this session's #96 assignment (P1-16) made those refs silently resolve to an unrelated ticket, worse than dangling. **P1-16 renumbered #96 → #98** with provenance on the task line; the **Id-reservations note extended** to dangling-reference targets (#96 now reserved until the #110/#128 renumber resolves/drops its refs; standing rule: grep before assigning any id). **(3) #69 coupling verified before writing** (not taken on the operator's word): `(use_full_panel or not lite)` at `cli.py:689` (inbox `fm_full`) and `:812` (`--file` `eff_full`, models-gate `not eff_full` at `:814`) IS the expression that makes `full_panel` the effective 5-model default — constraint added to #69: a parity fix changing its semantics silently flips the panel default 5→3; decouple and pin with a regression test first. **(4) Moratorium lift RECORDED** (gap 3 closed): the 2026-07-21 re-scope moratorium was lifted mid-window but never written down — now in the grooming log; **#4 re-scoped in place**: (a) ADR-02 overlap-policy amendment, (b) stale-stamp refresh, (c) panel-default explicitly DISSOLVED and fenced out of scope. **(5) #99 filed** ([S15]): pointer to the un-triaged batch proposal set (`2026-07-22-pre-handoff-cleanup.md` §2) so a BACKLOG-only reader sees it exists (gap 2 closed); individual triage deliberately deferred. **(6)** `scripts/council-ask.ps1` **deleted** (approved; batch §2.5 — referenced by nothing, removal verified on disk).

**Result:** validators 0/0/0/0; `validate_backlog` OK 7 themes / 15 stories / **59** tasks; next free id **#100** (reserved: #84, #85, #96). Canonical serial suite **818 passed + 1 xfailed + 6 deselected — green (61.34s)**. **Flake finding recorded, deliberately not filed:** first `-n auto` verification run failed once on `test_no_persist_removes_scratch_on_abort` — xdist cross-worker interference (the test snapshots the SHARED system temp; a concurrent worker's scratch dir landed between snapshots). Test passes alone; zero `aicouncil-scratch-*` leftovers on disk — no genuine leak. This is the suite's first witnessed non-xdist-safe test and further cements the "do NOT adopt `-n auto` in `check.ps1`" recommendation; it attaches to the §2 xdist thread #99 already points at — the triage session files it if wanted.

**Changes:** `BACKLOG.md` (renumber, reservations rule, #69 constraint, #4 re-scope, #99, grooming entry), `scripts/council-ask.ps1` deleted, this entry. Session merges: `95b9e4f` (pushed) + this branch's landing.

**Abandoned:** did NOT resolve/drop the `refs #96` text inside #110/#128 — that belongs to the pending #110→#84/#128→#85 renumber arc (audit A1's "same edit" instruction); the extended reservation covers the hazard meanwhile. Did not file the xdist flake (above).

**Next:** the #110→#84/#128→#85 renumber (now with the #96-ref resolution folded in); #99 triage session; #4's ADR-02 amendment.

### 2026-07-23 — Session close: absorb, file, seal (branch `chore/session-close-absorb`, STOPPED before merge per prompt)

**Did:** Filing-and-governance unit, zero implementation. **Block 1:** merged the night batch — `check.ps1` fully green first (818+1xf+6desel, mypy 41 files clean, ruff clean), `--no-ff` merge **`eb82fd5`**, pushed `8888d9e..eb82fd5`, merged branch deleted (approved). **Block 2 (archaeology, read-only): the panel-default "authority conflict" DISSOLVED.** `default_panel` has been 3-model since its introduction (`b513cad`, 2026-02-21) and ADR-02's Revised (2026-05-11) Implementation section explicitly documents the indirection: the 5-model effective default lives in `cli.py` — live code confirms `eff_full = (use_full_panel or not lite)` at `cli.py:689`/`:812`, so a bare run selects `full_panel` on both entry paths. Code, ADR-02, ARCHITECTURE, and the GUIDE are mutually consistent; the batch report's §2.1 (and the luna scan under it) read `determine_panel()` in isolation — **neither fork (a) nor (b); the finding was false.** Withdrawn via a dated AMENDMENT appended to `docs/audits/2026-07-22-pre-handoff-cleanup.md` (immutability-conform, additive; beyond-prompt decision, flagged), which also refines checker rule 5: config-claim parity must adjudicate **effective flag-resolution behaviour**, never raw config keys. #4's remaining scope = the ADR-02 overlap-policy amendment + its stale stamp (both already filed/known). **Block 3:** the 8 live-unfiled P1s filed after first-hand re-verification of each (none fixed incidentally): **#89/#90/#91** (P1-6/10/11, research reliability, [S3]) + new **[S17]** with **#92** (P1-12 `cli.run()` — ARCHITECTURE invariant 2 attached as the target, NOT reworded), **#93** (P1-13), **#94/#95/#96** (P1-14/15/16 test-trust). **Block 4:** new **[S18]/#97** — claim-vs-reality checker v1, twelve rules each traced to its drift, rule 12 (evidence commands mandatory) now backed by four scanner false positives. **Block 5:** `pytest-xdist>=3.8` declared in dev extras (installed-undeclared was itself rule-10 drift); **clean measurement, nothing else running: serial 61.25s vs `-n auto` 43.13s — 1.42×, both 818 passed** (the batch's 3.9× compared two contaminated numbers). **Recommendation: do NOT adopt `-n auto` in `check.ps1`** — 18s absolute gain does not buy the flakiness surface; the real lever is the e2e git-fixture families (report §2.11); keep xdist for local dev loops. **Block 6:** ADR-11 Deployment-Status refreshed by additive dated amendment marker only (#35 struck 2026-07-19 → remainder #34; the 2026-07-22 boost amendment post-dates the stamp). No deletions beyond the approved merged-branch cleanup. **Block 7 (all repo-record confirmations first-hand):** owner-C = ADR-11:68; ADR-95 boundary = ADR-11 §(e); #69 parity = `tests/test_boost.py:439` T8 **strict** xfail (XPASS errors the suite the moment boost wiring lands); #84/#85 reservation note live; checker spec = [S18]/#97; panel-default = dissolved, recorded in the report amendment + #97 + grooming log.

**Result:** validators 0/0/0/0 (`validate_backlog` OK 7 themes / 15 stories / 58 tasks); 825 collected. Backlog debt of three windows closed: count 49→58, stories 13→15, next free id **#98**, #84/#85 still reserved. **Fresh-architect gaps named (Block 7):** (1) the review-lane routing posture (luna/terra assignments, sol withdrawn on cost) is not a repo record — lives in `~/.claude` + this JOURNAL only, adjacent to #73's convention decision; (2) the batch report's un-triaged §2 proposals (council-ask.ps1, cache_invalidate/model_string, KEY-SEALED relocation, colors-branch deletion, duplication extractions, e2e-fixture optimization, lazy imports, fsck-arc prune) are findable in the dated audit but tracked nowhere — deliberate pending operator triage, but a BACKLOG-blind reader misses them; (3) whether the #4 re-scoping moratorium (2026-07-21 grooming) still holds is not derivable from the repo.

**Changes:** `BACKLOG.md` (+9 tasks, +2 stories, grooming entry), `pyproject.toml` (xdist declared), `docs/decisions/ADR-11-*.md` (additive stamp-refresh marker), `docs/audits/2026-07-22-pre-handoff-cleanup.md` (dated amendment), this entry. Session commits: `eb82fd5` (merge, pushed), `db05d0f`, `7f8d6bc` (+ this journal commit) on `chore/session-close-absorb` — **NOT merged; operator authorizes.**

**Abandoned:** No edit to #4's task line (2026-07-21 grooming holds re-scoping under moratorium; scope recorded in report/grooming instead). No `-n auto` in check.ps1 (measured against, above). No new audit document for this unit (standing signal: the diff is the deliverable; this entry + the amended batch report carry the record).

**Next:** operator: authorize/decline the `chore/session-close-absorb` merge; triage report-§2 residuals; rule on the #73 review-runner convention (would also absorb gap 1).
