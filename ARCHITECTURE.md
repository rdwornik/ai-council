...
- **`.\scripts\check.ps1`** — the pre-merge gate: `pytest` + `mypy` + `ruff`. Run before every merge (CLAUDE §5); not wired to pre-commit. A non-blocking #97 claim-vs-reality report (`scripts/validate_claims.py`) also runs as a section but does not gate. **Every tool runs through the repo venv explicitly** (`.venv\Scripts\python.exe -m <tool>`, #123): a bare `pytest`/`mypy`/`ruff` resolves to whatever is first on PATH, and the system interpreter carries its own editable `ai_council` install, so `import ai_council` resolved to the primary checkout from any working directory. The gate **fails loud when the venv is absent** rather than falling back to a bare runner — a silent fallback is how a run against the wrong tree looks identical to a correct one — and it runs from the repo root so relative targets cannot bind to a caller's checkout.
- **Validator reporting contract (ADR-16 clause 4):** a gating validator's **success** output states **what it checked and how many items** — an exit code alone is a verdict without its predicate, and exit 0 with zero bytes cannot be distinguished from a run that examined nothing. Each of `canonical_freshness`, `validate-docs-registry`, `validate-sealed-keys`, `validate-audit-casing` and `validate-backlog` prints one such line; `validate_claims` additionally discloses, per rule, the surface set that rule actually read. Homed by ADR-16 from BACKLOG #126, and **expires by reference** when the hub rules on validator reporting (the #111 precedent).
