...
- [#39] [P3][S] `--no-persist`/scratch output mode for witness/dev runs + bounded `output/health/` retention: every run writes canonical `output/` unconditionally (no dry-run branch), so witness/dev runs pollute `output/` and cleanup is manual (the night batch found ~15 un-cleaned `output/health/doctor-*.json` from the #16/#25 dev runs) · Done when: a witness run can be directed to a scratch/temp dir (flag or `AICOUNCIL_OUTPUT_DIR`) leaving canonical `output/` untouched, and `output/health/` retention is bounded/documented · refs `docs/audits/2026-07-17-night-batch-empirical-e2e-audit.md` §3.4 (guard gap), `output.py` `_write_routed`, `doctor.py` `write_record`
- [#40] [P3][S] Fix the verdict-package `options_considered` extractor: empty on pick verdicts (no synthesis heading matches `_OPTIONS_HEADING_MARKERS`) and populated-but-polluted on ideas (scoops endorsement/sub-bullet lines as options; leaks trailing `**`) · Done when: a pick verdict carries the panel's structured alternatives and an ideas verdict carries only top-level options with emphasis stripped · refs `docs/audits/2026-07-17-night-batch-empirical-e2e-audit.md` §3.2 (H2), `output.py` `_extracted_options`/`_OPTIONS_HEADING_MARKERS`; DRAFT-INT-1
...
- [#41] [P3][S] Fix the CLI-seat token-count regexes: codex `_TOKENS_RE` does not match codex 0.144.5's stderr banner (records `output_tokens: 0` though content is real), and claude `modelUsage.usage.input_tokens` under-reports prompt input (records ~9) · Done when: a CLI-seat call records its real token counts in the metrics sidecar (cost is $0 regardless; the totals must not skew) · refs `docs/audits/2026-07-17-night-batch-empirical-e2e-audit.md` §2.5 (F-M1/F-M2), `providers/cli_base.py` `CodexCliProvider._TOKENS_RE` + `ClaudeCliProvider._extract`
